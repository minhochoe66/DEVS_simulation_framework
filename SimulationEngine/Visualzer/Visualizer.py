...
class Visualizer:
    ...
    def __init__(self, engine, timeout):
        ...
        fig = plt.figure()
        plt.axes(xlim=(-10, 270), ylim=(0, 100))

        runner = SimulationRunnerForVisualizer(self.engine, plt, fig)
        anim = FuncAnimation(fig, runner.runSingleStep,
                             frames=timeout, interval=100, blit=True, repeat=False)
        plt.show()
...
class SimulationRunnerForVisualizer(object):
    def __init__(self, engine, plt, fig):
        ...
        self.txtTime = plt.text(5, 5, '', fontsize=12)
        self.lines = []
        for i in range(100):
            line, = plt.plot([], [])
    ...
    def runSingleStep(self, i):
        ...
        self.txtTime.set_text('Simulation Time : ' +
                              str(self.engine.getTime()))
