...
def plan(
    ...
    # no path if either endpoint lies inside an obstacle
    ...
    while open_pq:
        ...
        for v in neighbors(*u):
            if v in closed:
                continue
            # skip neighbours that lie inside an obstacle
            if not cell_is_free(*v):
                continue

            # Theta*: if the parent has line of sight, relax through the parent
            if parent[u] != u and line_free_cells(parent[u], v):
                ...
                # otherwise fall back to checking the u->v segment
